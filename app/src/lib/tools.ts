...
import type { Tool, JSONSchema } from '../types/webmcp';
import type {
	VenuesSearchParams,
...
async function requestApproval(toolName: string, input: unknown): Promise<unknown> {
	...
	const check = setInterval(() => {
		...
		if (outcome === 'approved') {
			...
			reject(new Error('User rejected the action'));
		}
	}, 300);
	return promise;
}
...
function tool(name: string, description: string, schema: JSONSchema, execute: (input: unknown) => Promise<unknown>): Tool {
	return { name, description, inputSchema: schema, execute };
}
...
export function availableTools(state: CityState): Tool[] {
	...
	tools.push(
		tool('search_venues', 'Search Agent City venues by capacity, price, and date. Returns matching venues with capacity, price, location, and features.', venueSearchSchema(), async (input) => {
		...
		tool('get_venue_details', 'Get full details for a specific venue by ID.', idSchema(), async (input) => apiFetch(`/venues/${readString(input, 'venueId') ?? ''}`)),

		tool('search_catering', 'Search catering packages by people count and dietary preference. Returns packages that match the diet and minimum order size.', cateringSearchSchema(), async (input) => {
		...
		tool('calculate_catering', 'Calculate the total price for a catering package at a given people count.', calculateCateringSchema(), async (input) =>
			apiFetch(`/catering/${readString(input, 'packageId') ?? ''}/calculate?people=${readNumber(input, 'people') ?? 1}`)
		),

		tool('find_available_slots', 'Find available calendar slots for a given date and time range.', slotsSchema(), async (input) => {
		...
		tool('get_event_plan', 'Return the current event plan including attendees, budget, chosen venue, catering, and calendar slot.', { type: 'object', properties: {} }, async () => ({ event: state.event })),

		tool('get_budget_status', 'Get the current budget status: limit, reserved, spent, remaining.', { type: 'object', properties: {} }, async () => apiFetch('/budget/status')),

		tool('calculate_event_cost', 'Calculate the total estimated cost for the current event plan with optional venue and catering overrides.', budgetCalculateSchema(), async (input) => {
		...
		tool('update_event_requirements', 'Update the high-level event requirements such as attendees, date, dietary preference, or budget limit.', updateRequirementsSchema(), async (input) => {
	...
	tools.push(
		tool('reserve_venue', 'Reserve a venue for the event. Requires human approval.', reserveVenueSchema(), async (input) => {
			if (needsApproval('reserve_venue')) await requestApproval('reserve_venue', input);
		...
		tool('place_catering_order', 'Place a catering order for the event. Requires human approval.', orderCateringSchema(), async (input) => {
			if (needsApproval('place_catering_order')) await requestApproval('place_catering_order', input);
		...
		tool('schedule_event', 'Schedule the event in a calendar slot. Requires human approval.', scheduleSchema(), async (input) => {
			if (needsApproval('schedule_event')) await requestApproval('schedule_event', input);
	...
	if (state.event.venue) {
		tools.push(
			tool('modify_reservation', 'Change the reserved venue to another venue. Useful when constraints change.', modifyReservationSchema(), async (input) => {
				const body: ReserveVenueBody = { venueId: readString(input, 'venueId') ?? '', attendees: readNumber(input, 'attendees') ?? state.event.attendees, date: readString(input, 'date') ?? state.event.date };
				const res = await apiFetch('/venues/reserve', { method: 'POST', body: JSON.stringify(body) });
				mutateFromResult(res);
				return res;
			...
			tool('cancel_reservation', 'Cancel the current venue reservation. Requires human approval.', { type: 'object', properties: {} }, async () => {
				if (needsApproval('cancel_reservation')) await requestApproval('cancel_reservation', {});
	...
	if (state.event.catering) {
		tools.push(
			tool('modify_catering_order', 'Change the catering order to another package or people count.', modifyCateringSchema(), async (input) => {
				const body: PlaceCateringBody = { packageId: readString(input, 'packageId') ?? '', people: readNumber(input, 'people') ?? state.event.attendees };
				const res = await apiFetch('/catering/order', { method: 'POST', body: JSON.stringify(body) });
				mutateFromResult(res);
				return res;
			...
			tool('cancel_catering_order', 'Cancel the current catering order. Requires human approval.', { type: 'object', properties: {} }, async () => {
				if (needsApproval('cancel_catering_order')) await requestApproval('cancel_catering_order', {});
	...
	if (state.event.calendarSlot) {
		tools.push(
			tool('reschedule_event', 'Move the event to a different calendar slot.', scheduleSchema(), async (input) => {
				const body: ScheduleEventBody = { slotId: readString(input, 'slotId') ?? '' };
			...
			tool('cancel_event', 'Cancel the scheduled event. Requires human approval.', { type: 'object', properties: {} }, async () => {
				if (needsApproval('cancel_event')) await requestApproval('cancel_event', {});
